...
for i = 1:length(t)
    ...
    % Cálculo de la fuerza requerida utilizando el controlador STSMC para el lider.     
    z_mphi2 = ez2 + k0_z*((abs(ezp2)^(3/2))*sign(ezp2));
    z32 = z32 +(-k2_z*sign(z_mphi2))*dt;
    z_bar2= -k1_z*((abs(z_mphi2)^(1/3))*sign(z_mphi2)) + z32;
    
    u2 =((z_bar2+g)/(cos(theta2)*cos(phi2)))*m;
    
    % Cálculo de la fuerza requerida utilizando el controlador STSMC para el seguidor 
    z_mphi1 = ez1 + k0_z*((abs(ezp1)^(3/2))*sign(ezp1));
    z31 = z31 +(-k2_z*sign(z_mphi1))*dt;
    z_bar1= -k1_z*((abs(z_mphi1)^(1/3))*sign(z_mphi1)) + z31;
    ...
    phi_mphi2 = ephi2 + k0_phi*((abs(ephip2)^(3/2))*sign(ephip2));
    phi32 = phi32 + (-k2_phi*sign(phi_mphi2))*dt;    
    tau_bar_phi2 = -k1_phi*((abs(phi_mphi2)^(1/3))*sign(phi_mphi2)) + phi32;

    tau_phi2=Jx*(tau_bar_phi2-((Jy-Jz)/Jx)*thetap2*psip2);

    % Control de Theta

    phi_mtheta2 = etheta2 + k0_theta*((abs(ethetap2)^(3/2))*sign(ethetap2));
    theta32 = theta32 + (-k2_theta*sign(phi_mtheta2))*dt;
    tau_bar_theta2 = -k1_theta*((abs(phi_mtheta2)^(1/3))*sign(phi_mtheta2)) + theta32;
    
    tau_theta2=Jy*(tau_bar_theta2-((Jz-Jx)/Jy)*phip2*psip2);
   
    % Control de Psi
    
    phi_mpsi2 = epsi2 + k0_psi*((abs(epsip2)^(3/2))*sign(epsip2));
    psi32 = psi32 + (-k2_psi*sign(phi_mpsi2))*dt;
    tau_bar_psi2 = -k1_psi*((abs(phi_mpsi2)^(1/3))*sign(phi_mpsi2)) + psi32;
    ...
    phi_mphi1 = ephi1 + k0_phi*((abs(ephip1)^(3/2))*sign(ephip1));
    phi31 = phi31 + (-k2_phi*sign(phi_mphi1))*dt;    
    tau_bar_phi1 = -k1_phi*((abs(phi_mphi1)^(1/3))*sign(phi_mphi1)) + phi31;

    tau_phi1=Jx*(tau_bar_phi1-((Jy-Jz)/Jx)*thetap1*psip1);

    % Control de Theta

    phi_mtheta1 = etheta1 + k0_theta*((abs(ethetap1)^(3/2))*sign(ethetap1));
    theta31 = theta31 + (-k2_theta*sign(phi_mtheta1))*dt;
    tau_bar_theta1 = -k1_theta*((abs(phi_mtheta1)^(1/3))*sign(phi_mtheta1)) + theta31;
    
    tau_theta1 = Jy*(tau_bar_theta1-((Jz-Jx)/Jy)*phip1*psip1);
   
    % Control de Psi
    
    phi_mpsi1 = epsi1 + k0_psi*((abs(epsip1)^(3/2))*sign(epsip1));
    psi31 = psi31 + (-k2_psi*sign(phi_mpsi1))*dt;
    tau_bar_psi1 = -k1_psi*((abs(phi_mpsi1)^(1/3))*sign(phi_mpsi1)) + psi31;
...
EX1_NTSMC     = EX1(pm:end);
EY1_NTSMC     = EY1(pm:end); 
EZ1_NTSMC     = EZ1(pm:end);
EPHI1_NTSMC   = EPHI1(pm:end);
ETHETA1_NTSMC = ETHETA1(pm:end);
EPSI1_NTSMC   = EPSI1(pm:end);

EX2_NTSMC = EX2(pm:end);
EY2_NTSMC = EY2(pm:end);
EZ2_NTSMC = EZ2(pm:end);
EPHI2_NTSMC = EPHI2(pm:end);
ETHETA2_NTSMC = ETHETA2(pm:end);
EPSI2_NTSMC = EPSI2(pm:end);


X1_NTSMC     = X1(pm:end);
Y1_NTSMC     = Y1(pm:end); 
Z1_NTSMC     = Z1(pm:end);
PHI1_NTSMC   = PHI1(pm:end);
THETA1_NTSMC = THETA1(pm:end);
PSI1_NTSMC   = PSI1(pm:end);

X2_NTSMC     = X2(pm:end);
Y2_NTSMC     = Y2(pm:end);
Z2_NTSMC     = Z2(pm:end);
PHI2_NTSMC   = PHI2(pm:end);
THETA2_NTSMC = THETA2(pm:end);
PSI2_NTSMC   = PSI2(pm:end);

XD1_NTSMC     = X1(pm:end);
YD1_NTSMC     = Y1(pm:end); 
ZD1_NTSMC     = Z1(pm:end);
PHID1_NTSMC   = PHI1(pm:end);
THETAD1_NTSMC = THETA1(pm:end);
PSID1_NTSMC   = PSI1(pm:end);

XD2_NTSMC     = XD2(pm:end);
YD2_NTSMC     = YD2(pm:end);
ZD2_NTSMC     = ZD2(pm:end);
PHID2_NTSMC   = PHID2(pm:end);
THETAD2_NTSMC = THETAD2(pm:end);
PSID2_NTSMC   = PSID2(pm:end);

UZ1_NTSMC       = UZ1(pm:end);
TAUPHI1_NTSMC   = TAUPHI1(pm:end); 
TAUTHETA1_NTSMC = TAUTHETA1(pm:end);
TAUPSI1_NTSMC   = TAUPSI1(pm:end);

UZ2_NTSMC       = UZ2(pm:end);
TAUPHI2_NTSMC   = TAUPHI2(pm:end); 
TAUTHETA2_NTSMC = TAUTHETA2(pm:end);
TAUPSI2_NTSMC   = TAUPSI2(pm:end);

save('NTSMC_Errores.mat','EX1_NTSMC','EX2_NTSMC','EY1_NTSMC','EY2_NTSMC','EZ1_NTSMC','EZ2_NTSMC','EPHI1_NTSMC','EPHI2_NTSMC','ETHETA1_NTSMC','ETHETA2_NTSMC','EPSI1_NTSMC','EPSI2_NTSMC','-v7.3');
save('NTSMC_Estados.mat','X1_NTSMC','X2_NTSMC','Y1_NTSMC','Y2_NTSMC','Z1_NTSMC','Z2_NTSMC','PHI1_NTSMC','PHI2_NTSMC','THETA1_NTSMC','THETA2_NTSMC','PSI1_NTSMC','PSI2_NTSMC','-v7.3');
save('NTSMC_Deseadas.mat','XD1_NTSMC','XD2_NTSMC','YD1_NTSMC','YD2_NTSMC','ZD1_NTSMC','ZD2_NTSMC','PHID1_NTSMC','PHID2_NTSMC','THETAD1_NTSMC','THETAD2_NTSMC','PSID1_NTSMC','PSID2_NTSMC','-v7.3')
save('NTSMC_Control.mat','UZ1_NTSMC','UZ2_NTSMC','TAUPHI1_NTSMC','TAUPHI2_NTSMC','TAUTHETA1_NTSMC','TAUTHETA2_NTSMC','TAUPSI1_NTSMC','TAUPSI2_NTSMC','t','-v7.3')
