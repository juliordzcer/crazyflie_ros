...
for i = 1:length(t)
    ...
    z_mphi = ezp + k0_z*((abs(ez)^(2/3))*sign(ez));
    z3 = z3 +(-k2_z*sign(z_mphi))*dt;
    z_bar= -k1_z*((abs(z_mphi)^(1/2))*sign(z_mphi)) + z3;
    
    u =((z_bar+g)/(cos(theta)*cos(phi)))*m;
    
    % Calculo de phi* y theta* 
    thetad2  = asin((kpx*ex +kdx*exp)*m/u); 
    phid2    = asin((kpy*ey +kdy*eyp)*m/u);
...
%     % Control de Phi
    ...
    phi_mth = ethetap + k0_theta*((abs(etheta)^(2/3))*sign(etheta));
    theta3 = theta3 + (-k2_theta*sign(phi_mth))*dt;
    tau_bar_theta = -k1_theta*((abs(phi_mth)^(1/2))*sign(phi_mth)) + theta3;
