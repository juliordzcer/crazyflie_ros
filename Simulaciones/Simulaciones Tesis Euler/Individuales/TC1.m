...
for i = 1:length(t)
    ...
    % Cálculo de la fuerza requerida utilizando el controlador TC 
    
    z3 = z3 +(-k3_z*sign(ez)-k4_z*sign(ezp))*dt;
    z_bar= -k1_z*(abs(ez)^(1/3))*sign(ez)-k2_z*(abs(ezp)^(1/2))*sign(ezp) + z3;
    
    um =(z_bar+g)/(cos(theta)*cos(phi));
    
    % Control de Phi
    
    phi3 = phi3 + (-k3_phi*sign(ephi)-k4_phi*sign(phip))*dt;    
    tau_bar_phi = -k1_phi*(abs(ephi)^(1/3))*sign(ephi)-k2_phi*(abs(phip)^(1/2))*sign(phip) + phi3;

    tau_phi=Jx*(tau_bar_phi-((Jy-Jz)/Jx)*thetap*psip);

    % Control de Theta

    theta3 = theta3 + (-k3_theta*sign(etheta) -k4_theta*sign(thetap))*dt;
    tau_bar_theta = -k1_theta*(abs(etheta)^(1/3))*sign(etheta)-k2_theta*(abs(thetap)^(1/2))*sign(thetap)+ theta3;
    
    tau_theta=Jy*(tau_bar_theta-((Jz-Jx)/Jy)*phip*psip);
   
    % Control de Psi
    
    psi3 = psi3 + (-k3_psi*sign(epsi)-k4_psi*sign(epsip))*dt;
    tau_bar_psi = -k1_psi*(abs(epsi)^(1/3))*sign(epsi)-k2_psi*(abs(epsip)^(1/2))*sign(epsip) + psi3;

    tau_psi = Jz*(tau_bar_psi-((Jx-Jy)/Jz)*thetap*phip);
    
    
    % Modelo dinamico del quadrotor
    xpp     = (um/m)*(cos(phi) * sin(theta)*cos(psi)+sin(phi)*sin(psi))-ax*xp+dx;
    ypp     = (um/m)*(cos(phi) * sin(theta)*sin(psi)-sin(phi)*cos(psi))-ay*yp+dy;
    zpp     = (um/m)*(cos(phi) * cos(theta))-g-az*zp+dz;
