...
for i = 1:length(t)
    ...
    % Cálculo de la fuerza requerida utilizando el controlador STA para el lider.     
    z_mphi2 = ezp2 + k0_z*ez2;
    z32 = z32 +(-k2_z*sign(z_mphi2))*dt;
    z_bar2= -k1_z*((abs(z_mphi2)^(1/2))*sign(z_mphi2)) + z32;
    
    u2 =((z_bar2+g)/(cos(theta2)*cos(phi2)))*m;
    
    % Cálculo de la fuerza requerida utilizando el controlador STA para el seguidor 
    z_mphi1 = ezp1 + k0_z*ez1;
    ...
    phi_mth2 = ethetap2 + k0_theta*etheta2;
    theta32 = theta32 + (-k2_theta*sign(phi_mth2))*dt;
    tau_bar_theta2 = -k1_theta*((abs(phi_mth2)^(1/2))*sign(phi_mth2)) + theta32;
    
    tau_theta2=Jy*(tau_bar_theta2-((Jz-Jx)/Jy)*phip2*psip2);
   
    % Control de Psi
    
    phi_mpsi2 = epsip2 + k0_psi*epsi2;
    ...
    phi_mth1 = ethetap1 + k0_theta*etheta1;
    theta31 = theta31 + (-k2_theta*sign(phi_mth1))*dt;
    tau_bar_theta1 = -k1_theta*((abs(phi_mth1)^(1/2))*sign(phi_mth1)) + theta31;
    
    tau_theta1 = Jy*(tau_bar_theta1-((Jz-Jx)/Jy)*phip1*psip1);
   
    % Control de Psi
    
    phi_mpsi1 = epsip1 + k0_psi*epsi1;
...
EY1_STA     = EY1(pm:end); 
EZ1_STA     = EZ1(pm:end);
EPHI1_STA   = EPHI1(pm:end);
ETHETA1_STA = ETHETA1(pm:end);
EPSI1_STA   = EPSI1(pm:end);

EX2_STA = EX2(pm:end);
EY2_STA = EY2(pm:end);
EZ2_STA = EZ2(pm:end);
EPHI2_STA = EPHI2(pm:end);
ETHETA2_STA = ETHETA2(pm:end);
EPSI2_STA = EPSI2(pm:end);


X1_STA     = X1(pm:end);
Y1_STA     = Y1(pm:end); 
Z1_STA     = Z1(pm:end);
PHI1_STA   = PHI1(pm:end);
THETA1_STA = THETA1(pm:end);
PSI1_STA   = PSI1(pm:end);

X2_STA     = X2(pm:end);
Y2_STA     = Y2(pm:end);
Z2_STA     = Z2(pm:end);
PHI2_STA   = PHI2(pm:end);
THETA2_STA = THETA2(pm:end);
PSI2_STA   = PSI2(pm:end);

XD1_STA     = X1(pm:end);
YD1_STA     = Y1(pm:end); 
ZD1_STA     = Z1(pm:end);
PHID1_STA   = PHI1(pm:end);
THETAD1_STA = THETA1(pm:end);
PSID1_STA   = PSI1(pm:end);

XD2_STA     = XD2(pm:end);
YD2_STA     = YD2(pm:end);
ZD2_STA     = ZD2(pm:end);
PHID2_STA   = PHID2(pm:end);
THETAD2_STA = THETAD2(pm:end);
PSID2_STA   = PSID2(pm:end);

UZ1_STA       = UZ1(pm:end);
TAUPHI1_STA   = TAUPHI1(pm:end); 
TAUTHETA1_STA = TAUTHETA1(pm:end);
TAUPSI1_STA   = TAUPSI1(pm:end);

UZ2_STA       = UZ2(pm:end);
TAUPHI2_STA   = TAUPHI2(pm:end); 
TAUTHETA2_STA = TAUTHETA2(pm:end);
TAUPSI2_STA   = TAUPSI2(pm:end);

save('STA_Errores.mat','EX1_STA','EX2_STA','EY1_STA','EY2_STA','EZ1_STA','EZ2_STA','EPHI1_STA','EPHI2_STA','ETHETA1_STA','ETHETA2_STA','EPSI1_STA','EPSI2_STA','t','-v7.3');
save('STA_Estados.mat','X1_STA','X2_STA','Y1_STA','Y2_STA','Z1_STA','Z2_STA','PHI1_STA','PHI2_STA','THETA1_STA','THETA2_STA','PSI1_STA','PSI2_STA','-v7.3');
save('STA_Deseadas.mat','XD1_STA','XD2_STA','YD1_STA','YD2_STA','ZD1_STA','ZD2_STA','PHID1_STA','PHID2_STA','THETAD1_STA','THETAD2_STA','PSID1_STA','PSID2_STA','-v7.3')
save('STA_Control.mat','UZ1_STA','UZ2_STA','TAUPHI1_STA','TAUPHI2_STA','TAUTHETA1_STA','TAUTHETA2_STA','TAUPSI1_STA','TAUPSI2_STA','-v7.3')
