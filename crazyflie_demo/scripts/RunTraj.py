...
class Demo():
    def __init__(self, goals):
        ...
        self.goals = goals
        self.goalIndex = 0
        self.pubGoalVelAce = rospy.Publisher('goal_velace', Twist, queue_size=1)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            goal_velace.linear.x = self.goals[self.goalIndex][4]
            goal_velace.linear.y = self.goals[self.goalIndex][5]
            goal_velace.linear.z = self.goals[self.goalIndex][6]

            goal_velace.angular.x = self.goals[self.goalIndex][8]
            goal_velace.angular.y = self.goals[self.goalIndex][9]
            goal_velace.angular.z = self.goals[self.goalIndex][10]

            self.pubGoalVelAce.publish(goal_velace)
            ...
            if self.listener.canTransform(self.worldFrame, self.frame, t):
                ...
                if     math.fabs(position[0] - self.goals[self.goalIndex][0]) < 0.6 \
                   and math.fabs(position[1] - self.goals[self.goalIndex][1]) < 0.6 \
                   and math.fabs(position[2] - self.goals[self.goalIndex][2]) < 0.6 \
                   and math.fabs(rpy[2] - self.goals[self.goalIndex][3]) < math.radians(10) \
                   and self.goalIndex < len(self.goals) - 1:
                        rospy.sleep(self.goals[self.goalIndex][12])
                        self.goalIndex += 1
