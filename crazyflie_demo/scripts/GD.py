...
class DataRecorder:
    ...
    def save_data(self):
        ...
        # Construir la ruta completa al archivo CSV y al archivo .mat para Quad2
        csv_path_quad2 = os.path.join(home_dir, self.base_name_2 + ".csv")
        mat_path_quad2 = os.path.join(home_dir, self.base_name_2 + ".mat")

        # Verificar si el archivo CSV ya existe para Quad2
        while os.path.exists(csv_path_quad2):
            self.contador += 1
            csv_path_quad2 = os.path.join(home_dir, self.base_name_2 + str(self.contador) + ".csv")
            mat_path_quad2 = os.path.join(home_dir, self.base_name_2 + str(self.contador) + ".mat")

        # Abrir el archivo CSV para Quad2
        csv_file_quad2 = open(csv_path_quad2, 'w')
        csv_writer_quad2 = csv.writer(csv_file_quad2)
        csv_writer_quad2.writerow(['x', 'y', 'z', 'phi', 'theta', 'psi', 'xd', 'yd', 'zd', 'phid', 'thetad', 'psid', 'u', 'tau_phi', 'tau_theta', 'tau_psi'])

        # Escribir los datos de Quad2 en el archivo CSV
        for data in self.Quad2:
            csv_writer_quad2.writerow(data)

        # Cerrar el archivo CSV para Quad2
        csv_file_quad2.close()
