...
{
public:
// tf_prefix es el nombre del robot en este caso el crazyflie, y este va aumentando dependiendo del numero de quadrotors
  CrazyflieROS(
    ...
    crazyflie_driver::sendPacket::Response &res)
  {
   ...
   /*  
    ---acc---  Corresponde a las se~nales del acelerometro
    ---gyro--- Corresponde a las se~nales del gyroscopio
    ---mag---  Corresponde a las se~nales del magnetrometro
    ---baro--- Corresponde a las se~nales del barometro
    ---pm---   vbat Corresponde a la bateria en volts
  */
private:
  struct logImu {
    float acc_x;
...
// Tiene un punto de ajuste de vuelo estacionario
...
void cmdHoverSetpoint(
    const crazyflie_driver::Hover::ConstPtr& msg)
  {
    ...
    if (!m_isEmergency) {
      ...
      m_sentSetpoint = true;
      //ROS_INFO("set a hover setpoint"); Establecer un punto de ajuste de desplazamiento
    }
  }
tf_prefix
