...
public:
  CrazyflieROS(
    ...
    , m_enable_logging_phi(enable_logging_pose)
    , m_enable_logging_theta(enable_logging_pose)
    , m_enable_logging_psi(enable_logging_pose)
    , m_serviceEmergency()
...
void cmdPositionSetpoint(
  ...
  void run()
  {
    ...
    if (m_enable_logging_phi) {
      m_pubPhi = n.advertise<std_msgs::Float32>(m_tf_prefix + "/phi", 10);
    }

    if (m_enable_logging_theta) {
      m_pubThetha = n.advertise<std_msgs::Float32>(m_tf_prefix + "/theta", 10);
    }

    if (m_enable_logging_psi) {
      m_pubPsi = n.advertise<std_msgs::Float32>(m_tf_prefix + "/psi", 10);
    }
    ...
    if (m_enableLogging) {
      ...
      if (   m_enable_logging_temperature
          ...
          || m_enable_logging_phi
          || m_enable_logging_theta
          || m_enable_logging_psi)
      {
  ...
  void onLog2Data(uint32_t time_in_ms, log2* data) {
    ...
    if (m_enable_logging_phi) {
      std_msgs::Float32 msg;
      // degrees
      msg.data = data->des_phi;
      m_pubPhi.publish(msg);
    }

    if (m_enable_logging_theta) {
      std_msgs::Float32 msg;
      // degrees
      msg.data = data->des_theta;
      m_pubThetha.publish(msg);
    }

    if (m_enable_logging_psi) {
      std_msgs::Float32 msg;
      // degrees
      msg.data = data->des_psi;
      m_pubPsi.publish(msg);
    }
  }
...
private:
  ...
  bool m_enable_logging_phi;
  bool m_enable_logging_theta;
  bool m_enable_logging_psi;
  ...
  ros::Publisher m_pubPhi;
  ros::Publisher m_pubThetha;
  ros::Publisher m_pubPsi;
